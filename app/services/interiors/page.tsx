...
import { ShieldCheck, Building2, CheckCircle2, ChevronRight, PhoneCall, Palette } from 'lucide-react';
...
export default function InteriorsServicePage() {
  ...
  useEffect(() => {
    async function fetchServiceDetail() {
      try {
        const res = await fetch('/api/services/interiors');
        ...
        console.error('Erreur chargement service Interiors:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F2EDE4] text-[#2C181A] flex flex-col items-center justify-center p-6">
        <p className="text-sm text-[#8C6D53]">Loading Interior &amp; Fit-Out data from database...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#F2EDE4] text-[#2C181A] flex flex-col items-center justify-center p-6 text-center">
        ...
        <div className="mt-20">
          ...
          <p className="text-xs text-[#8C6D53] mb-6">Please configure the Interiors service entry in your database.</p>
  ...
  let galleryImages = ['https://images.unsplash.com/photo-1618221195710-dd6b41faaea6'];
  try {
  ...
  let servicesGrid = [];
  try {
    if (serviceData.services_grid) {
      servicesGrid = typeof serviceData.services_grid === 'string' ? JSON.parse(serviceData.services_grid) : serviceData.services_grid;
    }
  } catch (e) {}
  ...
  return (
    <div className="min-h-screen bg-[#F2EDE4] text-[#2C181A] font-sans selection:bg-[#4A151B] selection:text-[#F2EDE4]">
      ...
      <section className="relative bg-[#4A1F23] text-[#F5E1C7] py-28 px-6 border-b border-[#E7B6A5]/20 pt-36 overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=1600&auto=format&fit=crop')] bg-cover bg-center opacity-20" />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex items-center gap-2 text-xs text-[#E7B6A5] mb-6">
            ...
            <ChevronRight className="w-3.5 h-3.5 opacity-60" />
            <span className="text-white">Interiors &amp; Fit-Out</span>
          </div>

          <span className="text-xs uppercase tracking-widest text-[#E7B6A5] font-semibold mb-3 block">Turnkey Design &amp; Build</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-6 text-[#685248] font-light leading-relaxed text-sm md:text-base">
              ...
              <h2 className="text-2xl text-[#4A1F23] font-serif">Nordic by Oravya — Architectural &amp; Design Excellence</h2>
              ...
              <div className="relative h-72 md:h-[380px] rounded-2xl overflow-hidden my-6 shadow-sm bg-[#DFD6C9]">
                <Image src={galleryImages[0]} alt="Interior Design and Fit-Out Dubai" fill className="object-cover" />
              </div>

              {/* BANDE BURGUNDY POUR METTRE EN VALEUR LA VISION */}
              {serviceData.vision && (
                <div className="my-8 p-8 bg-[#4A1F23] text-[#F5E1C7] rounded-2xl shadow-md border-l-4 border-[#C5A880]">
                  <span className="text-[10px] uppercase tracking-widest text-[#E7B6A5] font-bold block mb-2">Design Philosophy</span>
              ...
              <h3 className="text-xl text-[#4A1F23] font-serif pt-4 mb-4">Under-One-Roof Turnkey Capabilities</h3>

              {servicesGrid.length > 0 && (
                <div className="space-y-6">
                  {servicesGrid.map((item: any, idx: number) => (
                    <div key={idx} className="pb-6 border-b border-[#E7B6A5]/30">
                      <h4 className="text-base font-bold text-[#4A1F23] mb-1 font-serif">{idx + 1}. {item.title}</h4>
                      <p className="text-xs md:text-sm text-[#685248] leading-relaxed">{item.desc}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="bg-white border border-[#E7B6A5]/60 p-8 rounded-3xl shadow-xl sticky top-28">
            {inquirySent ? (
              <div className="py-8 text-center animate-in fade-in">
                ...
                <p className="text-xs text-[#8C6D53] font-light mb-6 leading-relaxed">
                  Thank you {clientName}. Our interior design and fit-out consultancy team will reach out to discuss your project specifications.
                </p>
                <button
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <h3 className="text-xl text-[#4A1F23] mb-1 font-serif">Transform Your Space</h3>
                <p className="text-xs text-[#8C6D53] mb-6">
                  Speak with our master architects and interior designers for a turnkey consultation.
                </p>
                ...
                <div>
                  <label className="block text-[11px] uppercase tracking-wider text-[#8C6D53] font-semibold mb-1">Project Details &amp; Scope</label>
                  <textarea
                    rows={3}
                    placeholder="Villa, apartment, commercial fit-out requirements..."
                    value={clientNotes}
            ...
            <div className="mt-6 pt-6 border-t border-[#E7B6A5]/30 flex items-center gap-3 text-xs text-[#8C6D53]">
              ...
              <span>Turnkey execution from architectural concept to final light fittings.</span>
            </div>
          </div>
